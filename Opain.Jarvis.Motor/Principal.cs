...
{
    internal class Principal : SX.Core.MotorBase.Base.MotorBase
    {
        public void Iniciar()
        {
            List<Func<System.Threading.CancellationToken, Task>> Tareas = new List<Func<System.Threading.CancellationToken, Task>>
            {
                Opain.Jarvis.Motor.Tareas.TareaEjemplo.Ejecutar,
                Opain.Jarvis.Motor.Tareas.TareaTest.Ejecutar
            };
                ...
                while (!base.CancellationTokenTareaMonitorizacion.IsCancellationRequested)
                {
                    foreach (Task laTarea in this.Tareas)
                    {
                        ...
                        if (laTarea.Exception.InnerException != null)
                            Diagnostics.EventLog.EscribirExcepcionNoControlada(laTarea.Exception.InnerException);
                        else
                            Diagnostics.EventLog.EscribirExcepcionNoControlada(laTarea.Exception);
                        #endregion
