...
namespace Opain.Jarvis.Tareas
{
    public class Ciudades
    {
        protected static async Task<int> Iniciar(int Ejecucion)
        {
            try
            {
                const string NombreInternoTareaConfigurada = "TraerCiudades";
                bool respuesta;
                ...
                if (HoraActual.ToString() == Tarea.Extension)
                {
                    if (Ejecucion < Tarea.IntervaloCantidadAProcesar)
                    {
                        ...
                        //Logica
                        respuesta = await servicio.SincronizarMaestra("api/Integracion/TraerCiudades"); //+ DateTime.Now.AddDays(-1).ToString("dd/MM/yyyy") );                       
                        Integracion.General.SystemTrace.Save(1, JsonConvert.SerializeObject(respuesta), 9004);
                    }
                    else
                    {
                        Integracion.General.SystemTrace.Save(2, "Se cumplió el límite de ejecuciones para hoy", 9004);
                    }
                }
                else
                {
        ...
        public static async Task Ejecutar(System.Threading.CancellationToken cancellationToken)
        {
            ...
            Diagnostico.EventLog.EscribirInformacion("Inicio del hilo de traer las Ciudades desde JDEdwars.");
            ...
            Diagnostico.EventLog.EscribirInformacion("Fin del hilo de traer las Ciudades desde JDEdwars.");
            await Task.Delay(TimeSpan.FromSeconds(5));
        }
    }
}
